...
for (let i = 0; i < args.length; i++) {
  switch (args[i]) {
    // when running inside a dev environment, specifically @substrate-dev/node-dev
    case '--dev-build':
      isDev = true;
      break;
...
try {
  const allFlags = `${importPath('@substrate/node-dev/scripts/substrate-exec-node-test.mjs')} ${[...cmd, ...files].join(' ')}`;

  nodeFlags.push('--require');
  nodeFlags.push(
    isDev
      ? `./packages/dev-test/build/cjs/${testEnv}.js`
      : `@substrate-dev/dev-test/${testEnv}`
  );
