...
function buildWebpack () {
  ...
  execPm(`substrate-exec-webpack --config ${config} --mode production`);
}
...
/**
 * Adjust all @substrate imports to have .ts extensions (for Deno)
 *
 * @param {string} pkgCwd
...
function adjustDenoPath (pkgCwd, pkgJson, dir, f, isDeclare) {
  if (f.startsWith('@substrate')) {
    ...
    const pjsPath = `${DENO_POL_PRE}/${thisPkg.replace('@substrate/', '')}`;
  ...
  readdirSync('build/cjs', ['.js']).forEach((thisPath) => {
    fs.writeFileSync(
      ...
      fs
        ...
        // .replace(
        //   /require\("@substrate\/([a-z-]*)\/(.*)"\)/g,
        //   'require("@substrate/$1/cjs/$2")'
        // )
        .replace(
          /require\("@substrate\/([a-z-]*)\/packageInfo"\)/g,
          'require("@substrate/$1/cjs/packageInfo")'
        )
    );
  });
}
...
function extractPackageInfoImports (filepath, withDetectImport) {
  ...
  const otherImports = withDetectImport
    ? ["import { detectPackage } from '@substrate/util';"]
    : [];
  /** @type {string[]} */
  const otherNames = [];
...
async function buildJs (compileType, repoPath, dir, locals) {
  ...
  if (!fs.existsSync(path.join(process.cwd(), '.skip-build'))) {
    ...
    if (!name.startsWith('@substrate/x-')) {
      if (!name.startsWith('@substrate-dev/node-dev')) {
        ...
        const withDetectImport = name !== '@substrate/util';
        ...
        if (withDetectImport) {
          // for @substrate/util this file contains the detection logic, keep it
...
async function main () {
  ...
  execPm('substrate-dev-clean-build');
  ...
  execPm('substrate-exec-tsc --build tsconfig.build.json');
  ...
  if (RL_CONFIGS.some((c) => fs.existsSync(path.join(process.cwd(), c)))) {
    execPm('substrate-exec-rollup --config');
  }
